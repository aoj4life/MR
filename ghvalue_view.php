...
if ($_SESSION['box45']==0) {
 ...
 if (!preg_match("/^[0-9]*$/", $_SESSION['bonusb'])){
  ...
  $sql_prevent = "UPDATE givedonations SET status='cashout' WHERE email='$email' AND status='comfirmed'";
  $resultqp = mysqli_query($con,$sql_prevent); 

 /* $matched = "INSERT INTO matched (tID, donoremail, donationvalue, receiveremail, receivervalue,) VALUES('$tid','r@r', '10000', 'f@f', '10000',)";

  $test1 = "INSERT INTO `matched` (`tID`, `donoremail`, `donationvalue`, `receiveremail`, `receivervalue`,`status`, `due_date`) VALUES ('888787', 'pop@adebayo', '42000', 'final@final', '42000','cashout_open', '2017-02-12 05:17:17')";
    $rem25 = mysqli_query($con, $test1); 

  //matching start here
     $sqlm = "SELECT * FROM givedonations WHERE status='unconfirmed' AND email!='email' ORDER BY time ASC limit 2"; 
     $rem = mysqli_query($con,$sqlm); 
     //$rowm = mysqli_fetch_row($rem);
       

      while($rowm = mysqli_fetch_array($rem))
        {
          $amount = 10000;
          $donoremail = $rowm[2];
          $donorvalue = $rowm[5];

     $matched ="INSERT INTO matched (tID, donoremail, donationvalue, receiveremail, receivervalue, due_date) VALUES ('55','r@r', '10000', 'f@f', '10000', '2017-02-15 05:17:17')";
          $rem25 = mysqli_query($con, $matched); 
   ...
   /* if($rem){
         ...
         $matched ="INSERT INTO matched (`tID`, `donoremail`, `donationvalue`, `receiveremail`, `receivervalue`, `due_date`) VALUES ('888787', '$rowm[2]', '$rowm[5]', '$email', '$amount', '2017-02-12 05:17:17')";
