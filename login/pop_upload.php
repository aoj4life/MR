...
$target_file = htmlspecialchars($target_file, ENT_QUOTES, 'UTF-8');
$target_file = htmlentities($target_file);

 $target_file = strip_tags(trim($target_file));
...
// if everything is ok, try to upload file
    ...
    if (move_uploaded_file($_FILES['fileToUpload']['tmp_name'],$target_dir.$target_file)) {
        ...
        //$sql2_query="SELECT * FROM matched WHERE donoremail='$email' AND status='cashout_open' ORDER BY datecreated ASC";
       // $result_set=mysqli_query($con,$sql2_query);
       // $row=mysqli_fetch_array($result_set);


       $sql1="UPDATE matched SET pop='$target_file' WHERE donoremail='$email' AND status='cashout_open' AND id='$_SESSION[tid22]'";
